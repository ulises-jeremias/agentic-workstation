...
Since ADR-009 (keep name, refresh tagline) was accepted, the dots-ai ecosystem has grown significantly. The companion workspace (`ai-workspace`) already decided in its ADR-0001 to rename to `agentic-harness`, but that rename was never executed because the two repos' identities are intertwined.

Several factors now favor revisiting the keep-name decision:

1. **Cross-repo naming friction** — The `dots-ai-workspace-knowledge-sync` skill literally combines both old names. Renaming one repo without the other would leave it disonant.
...
Rename the repository from `dots-ai` to `agentic-workstation` with the following scope:

### What changes

| Artifact | Old | New |
|----------|-----|-----|
| GitHub repo | `ulises-jeremias/dots-ai` | `ulises-jeremias/agentic-workstation` |
| chezmoi source dir | `~/.dots-ai` | `~/.agentic-workstation` |
| Install data dir | `~/.local/share/dots-ai` | `~/.local/share/agentic-workstation` |
| Config dir | `~/.config/dots-ai` | `~/.config/agentic-workstation` |
| Library dir | `~/.local/lib/dots-ai` | `~/.local/lib/agentic-workstation` |
| chezmoi config file | `~/.config/chezmoi/dots-ai.toml` | `~/.config/chezmoi/agentic-workstation.toml` |
...
- Cross-repo skill `dots-ai-workspace-knowledge-sync` can cleanly rename to `dots-harness-knowledge-sync`
...
- Install paths (`~/.local/share/dots-ai/`, `~/.config/dots-ai/`) require migration scripts
- chezmoi source state path changes — existing `~/.dots-ai/` must be moved
- External integrations referencing `ulises-jeremias/dots-ai` need updates
