...
`~/.local/share/agentic-workstation/mcp/linear/` for the dots-workstation-managed template.
...
3. agentic-workstation template available at `~/.local/share/agentic-workstation/mcp/linear/`
   (deployed by `chezmoi apply`).

## Setup (per AI tool)

The Linear MCP is a **streamable HTTP** endpoint with OAuth. Register it in
your AI tool of choice using the agentic-workstation template as reference:
...
Copy `~/.local/share/agentic-workstation/mcp/linear/config.template.json` into the right
