---
name: dots-workstation-pr-fallback
description: >-
...
- Remote is **GitHub** and **`dots-workstation-assistant`** (or a file search) shows **no** `PULL_REQUEST_TEMPLATE` / `pull_request_template` under `.github/`.
- You need a **reasonable default** for `gh pr create --body-file` until the repo adds its own template.

## Instructions

1. Run **`dots-workstation-output-handshake`**: user confirms where the final description will live (e.g. `body.md` for `gh`, paste-only) and that a **human** will review.
