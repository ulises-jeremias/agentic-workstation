...
from an environment variable. agentic-workstation ships a tool-agnostic template at
`~/.local/share/agentic-workstation/mcp/figma/` (deployed by `chezmoi apply`).
...
3. Store it persistently in `~/.config/agentic-workstation/env.d/` (covered by `dots-loadenv`):
...
`~/.local/share/agentic-workstation/mcp/figma/config.template.json` as reference:
...
1. Run `dots-doctor` and confirm `~/.config/agentic-workstation/env.d/figma.env` is listed
   under Integrations.
