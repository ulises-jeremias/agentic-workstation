...
3. **Run the analyzer:**
   ...
   ```bash
   python3 ~/.local/share/agentic-workstation/skills/gh-fix-ci/scripts/inspect_pr_checks.py \
...
6. **Draft a plan.** Propose the fix as a short, numbered plan. **Do not edit
   files yet.** If the project has a planning skill (e.g. `dots-workstation-planning`),
