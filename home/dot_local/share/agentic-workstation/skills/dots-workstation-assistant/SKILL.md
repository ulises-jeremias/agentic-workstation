...
description: agentic-workstation Assistant — on any repo, scan README→docs→AGENTS→CONTRIBUTING→PR templates→task runners→devcontainer→CI→configs before code; cite sources; prefer AGENTS.md for agent behavior; portable across Cursor/Copilot/Claude; org dots-* routing when needed.
metadata:
  author: agentic-workstation
  version: "2.1"
---

# agentic-workstation Assistant

Organizational companion for anyone building **for agentic-workstation** in **any** repository (client, internal, or `agentic-workstation`). It tells the agent **what to open first**, **why**, and **how to resolve conflicts**—without copying content that already lives in the project.

**Orchestration**

This skill is the **default orchestrator and fallback** for agentic-workstation agent work. **Read** `references/ORCHESTRATION.md` for routing (workflows, tool skills, delegation phrasing). Use **`~/.local/share/agentic-workstation/skills/skill-catalog.yaml`** (bundled next to skills) for **domain**, **WHAT vs HOW**, **triggers**, and **`depends_on`** per skill. **Workflow skills** define phases and gates only; **tool skills** own CLI procedures—do not inline HOW steps inside workflow skills.

**Hard rules**

- Derive answers from **files in the repo** and **machine-local agentic-workstation baseline** (`dots-*`, `~/.local/share/agentic-workstation/`). **Cite paths** when you recommend workflows.
...
## agentic-workstation workstation baseline (optional layer)

When the open repo **is** (or includes) `agentic-workstation` / chezmoi `home/`:

- Read repo root **`AGENTS.md`**, **`docs/*.md`**, **`home/.chezmoidata/*`**, **`home/.chezmoiscripts/*.tmpl`**, **`home/dot_local/share/agentic-workstation/.chezmoiexternal.toml.tmpl`** as needed.
- For **`dots-*` flags**, run `dots-<tool> --help`; do not cache outdated help text in answers.

On **any** machine with the baseline applied:

- **`~/.local/share/agentic-workstation/`** — bundled skills, templates, MCP examples, `skills-registry.yaml`.
...
| Update workstation | `chezmoi update && chezmoi apply --source=. -c ~/.config/chezmoi/agentic-workstation.toml` |
| List AI skills | `dots-skills list` |
| Sync skills | `dots-skills sync` |
| Health check | **`dots-workstation-triage`** (runs **`dots-doctor`**, layout checks) |

### Where to route next (bundled HOW skills)

Use **`skill-catalog.yaml`** next to bundled skills for domains, triggers and **`depends_on`**. Prefer **one workflow driver** per task (**dots-workstation-dev-companion** + **dots-workstation-workflow-generic-project** for generic delivery).

| Area | Skill(s) | Notes |
| --- | --- | --- |
| GitHub PR review threads | **`gh-address-comments`** | Current-branch PR; pairs with **`github-cli-workflow`** for push/PR updates |
| GitHub Actions failures | **`gh-fix-ci`** | Fetch logs + snippet; plan before coding; pairs with **`dots-workstation-planning`** |
| Draft PR / MR | **`github-cli-workflow`**, **`gitlab-cli-workflow`** | After **`dots-workstation-output-handshake`** / **`dots-workstation-pr-fallback`** when needed |
| Linear | **`linear`** | Linear MCP (OAuth); issues, cycles, docs |
| Figma | **`figma`** (entry), **`figma-implement-design`**, **`figma-code-connect-components`**, **`figma-create-design-system-rules`**, **`figma-create-new-file`** | MCP templates under `~/.local/share/agentic-workstation/mcp/figma/` |
...
| Playwright **test** specs | **`dots-workstation-e2e-runner`** | Not the same as **`playwright-cli`** |
| Jupyter notebooks | **`jupyter-notebook`** | Scaffold via **`dots-newnotebook`** |
| Workstation health | **`dots-workstation-triage`** | **`dots-doctor`**, **`dots-doctor --issue`** |
...
For AI agent assistance, invoke agents using **@mention** in your message (NOT the Task tool): **@dots-workstation-planner** (feature planning), **@dots-workstation-code-reviewer** (code review), **@dots-workstation-security-reviewer** (security), **@dots-workstation-tdd-guide** (TDD workflow), **@dots-workstation-reference-lookup** (agentic-workstation examples from public examples). Agents are defined in `~/.config/opencode/agents/` — they are NOT skills and must NOT be loaded via the skill tool.

---

## Organization playbooks

Client delivery workflows and the skill catalog live under **`~/.local/share/agentic-workstation/skills/`**. **Read** what is installed; do not invent URLs.

---

## Safety

- No secrets in repos; use env patterns documented for the project and agentic-workstation `env.d` where applicable.
