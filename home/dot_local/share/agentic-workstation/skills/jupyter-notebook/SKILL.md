...
```bash
python3 ~/.local/share/agentic-workstation/skills/jupyter-notebook/scripts/new_notebook.py \
  --kind experiment \
  --title "Compare prompt variants" \
  --out output/jupyter-notebook/compare-prompt-variants.ipynb
```
...
```bash
uv run --python 3.12 python ~/.local/share/agentic-workstation/skills/jupyter-notebook/scripts/new_notebook.py \
  --kind tutorial \
  --title "Intro to embeddings"
```
