...
1. Apply **`dots-workstation-output-handshake`**: confirm **where** the final ADR will be recorded and that a **human** will review.
...
1. **Confirm the decision qualifies** (new service, tech selection, schema change, deployment change, etc.) per the agentic-workstation "When to Create an ADR" criteria.
...
- `dots-workstation-decision-log` — lightweight decisions
- `dots-workstation-agreement` — explicit commitments or terms
- `dots-workstation-workflow-generic-project` — traceability and plan approval
