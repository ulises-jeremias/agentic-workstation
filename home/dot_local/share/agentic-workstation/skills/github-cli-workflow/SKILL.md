...
2. Prefer the repository **PR template** for the body: use `--body-file` pointing at a generated file that includes template sections, or paste from `.github/pull_request_template.md` when required. If no template exists in the repo, use **`dots-workstation-pr-fallback`** (WHAT) to build the body and **`dots-workstation-output-handshake`** to confirm where the file lives and that a human will review before `gh pr create`.
3. Create a **draft** PR:
