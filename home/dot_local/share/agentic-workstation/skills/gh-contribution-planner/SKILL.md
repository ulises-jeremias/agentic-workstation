...
3. **Run the analyzer.**
   ...
   ```bash
   python3 ~/.local/share/agentic-workstation/skills/gh-contribution-planner/scripts/inspect_contributions.py \
...
- **Isolation:** each sub-agent works in its own git worktree or a fresh clone
  under `~/.cache/agentic-workstation/contribution-planner/<owner>-<repo>/`. Never reuse
...
- Smoke test:
  ...
  ```bash
  python3 ~/.local/share/agentic-workstation/skills/gh-contribution-planner/scripts/inspect_contributions.py --dry-run
  ```
