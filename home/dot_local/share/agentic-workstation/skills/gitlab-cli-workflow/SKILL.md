...
2. Create an MR body file: use the project’s **merge request template** when present. If the project has no template, you may align the **Markdown** with **`dots-workstation-pr-fallback`** (same default sections as the org PR body) and **`dots-workstation-output-handshake`** for where the description file lives and human review—then use that file for `--description` or the equivalent.
