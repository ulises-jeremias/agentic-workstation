...
update_remote() {
  local dir="$1" label="$2"
  if [[ ! -d "${dir}/.git" ]]; then
    skip "${label}: not a git clone"
    return
  fi
  local url
  url="$(git -C "${dir}" remote get-url origin 2>/dev/null || true)"
  [[ -z ${url} ]] && {
    skip "${label}: no origin remote"
    return
  }
  local new_url=""
  ...
  [[ -z ${new_url} ]] && {
    skip "${label}: remote ${url} does not match ${MIGRATE_FROM}"
    return
  }
  local nb="${new_url%.git}" rb="${url%.git}"
  [[ ${rb} == "${nb}" ]] && {
    skip "${label}: already points to ${MIGRATE_TO}"
    return
  }
...
migrate_dir() {
  ...
  [[ -e ${dst} ]] && {
    skip "${src} -> ${dst}: destination already exists"
    return
  }
  mkdir -p "$(dirname "${dst}")"
  info "Moving ${src} -> ${dst}"
...
update_chezmoi_config() {
  [[ ! -f ${CHEZMOI_CONFIG_OLD} ]] && {
    skip "chezmoi config ${CHEZMOI_CONFIG_OLD}: not found"
    return
  }
  [[ -f ${CHEZMOI_CONFIG_NEW} ]] && {
    skip "chezmoi config ${CHEZMOI_CONFIG_NEW}: already exists"
    return
  }
